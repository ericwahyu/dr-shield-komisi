    ...
    <div class="card">
        ...
        <div class="table-responsive scrollbar-x">
            <table class="table table-bordered">
                ...
                <tbody>
                    @forelse ($user as $result)
                        <tr>
                            <td class="text-center">{{ $user?->currentPage() * $perPage - $perPage + $loop->iteration }}</td>
        ...
        <div class="card-body">
            <div class="row align-items-center g-3">
                <div class="col-lg-6 col-12">Menampilkan {{ $user->firstItem() }} sampai {{ $user->lastItem() }} dari {{ $user->total() }} hasil</div>
                <div class="col-lg-6 col-12 text-align-end">{{ $user->onEachSide(1)->links('vendor.livewire.custom-paginate') }}</div>
            </div>
        </div>
    </div>
</div>
