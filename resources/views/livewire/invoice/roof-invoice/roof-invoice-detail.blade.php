...
<div>
    ...
    <div class="d-flex align-items-center">
        <a href="{{ route('roof.invoice') }}" class="btn btn-icon" style="margin-right: 15px"><i
        ...
        <ul class="nav nav-pills mb-4 gap-3" role="tablist">
            <li class="nav-item" role="presentation">
                <button type="button" class="nav-link waves-effect waves-light" role="tab" data-bs-toggle="tab"
                    data-bs-target="#version-1" aria-controls="version-1" aria-selected="true">Versi 1</button>
            </li>
            <li class="nav-item" role="presentation">
                <button type="button" class="nav-link active waves-effect waves-light" role="tab"
                    data-bs-toggle="tab" data-bs-target="#version-2" aria-controls="version-2" aria-selected="false"
                    tabindex="-1">Versi 2</button>
            </li>
            {{-- <li class="nav-item" role="presentation">
    ...
    <div class="tab-content" style="border-radius: none; background: none; box-shadow: none; padding: 0rem">
                            ...
                            <div class="row g-3">
                                @foreach ($categories as $category)
                                    <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                        ...
                                        <input type="text" class="form-control"
                                            value="Rp. {{ number_format($amounts[$category?->slug], 2, ',', '.') }}"
                                ...
                                @foreach ($categories as $category)
                                    <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                        ...
                                        <input type="text" class="form-control"
                                            value="{{ $payment_amounts[$category?->slug] }}" placeholder="" disabled>
                                    </div>
                                @endforeach
                                @foreach ($categories as $category)
                                    <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                        ...
                                        <input type="text" class="form-control"
                                            value="{{ $remaining_amounts[$category?->slug] }}" placeholder=""
                                            disabled>
                                    </div>
                                @endforeach
                            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <button class="btn btn-danger btn-sm"
                                                    wire:click="deleteConfirm('{{ $result?->id }}')"
                                                    x-data="{ tooltip: 'Hapus' }" x-tooltip="tooltip"><i
                                                        class="fa-solid fa-trash-alt fa-fw"></i></button>
                                                <button class="btn btn-warning btn-sm"
                                                    wire:click="edit('{{ $result?->id }}')" x-data="{ tooltip: 'Edit' }"
                                                    x-tooltip="tooltip"><i
                                                        class="fa-solid fa-pencil-alt fa-fw"></i></button>
        ...
        <div class="tab-pane fade show active" id="version-2" role="tabpanel">
            ...
            <livewire:invoice.roof-invoice.roof-invoice-detail.roof-invoice-detail-v2 :id="$get_invoice?->id"
                wire:key="roof-invoice-detail-v2-{{ $get_invoice?->id }}" />
        </div>
    </div>
</div>
@push('scripts')
