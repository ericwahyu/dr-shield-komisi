...
<div>
    ...
    @include('livewire.invoice.roof-invoice.roof-invoice-detail.roof-invoice-detail-modal-v2')
    <div class="row">
        <div class="col-md-7">
            <div class="mb-4 card">
                    ...
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control @error('date') is-invalid @enderror"
                                wire:model.live="date" placeholder="" disabled>
                            @error('date')
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control @error('invoice_number') is-invalid @enderror"
                                wire:model="invoice_number" placeholder="" disabled>
                        ...
                        <div class="col-3">
                            ...
                            <input type="text" class="form-control @error('sales_code') is-invalid @enderror"
                                wire:model.live="sales_code" placeholder="" disabled>
                            @error('sales_code')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-3">
                            ...
                            <input type="text" class="form-control @error('due_date') is-invalid @enderror"
                                wire:model="due_date" min="0" placeholder="" disabled>
                            @error('due_date')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-3">
                            ...
                            <input type="text" class="form-control @error('customer') is-invalid @enderror"
                                wire:model="customer" placeholder="" disabled>
                            @error('customer')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-3">
                            ...
                            <input type="text" class="form-control @error('id_customer') is-invalid @enderror"
                                wire:model="id_customer" placeholder="" disabled>
                            @error('id_customer')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-4">
                            ...
                            <input type="text" class="form-control @error('income_tax') is-invalid @enderror"
                                wire:model.live="income_tax" min="0" placeholder="" disabled>
                            @error('income_tax')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-4">
                            ...
                            <input type="text" class="form-control @error('value_tax') is-invalid @enderror"
                                wire:model="value_tax" min="0" placeholder="" disabled>
                            @error('value_tax')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-4">
                            ...
                            <input type="text" class="form-control @error('amount') is-invalid @enderror"
                                wire:model="amount" min="0" placeholder="" disabled>
                            @error('amount')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        @foreach ($due_date_roof_rules as $key => $due_date_roof_rule)
                            ...
                            <div class="col-4">
                                ...
                                <input type="text" class="form-control"
                                    value="{{ Carbon::parse($get_invoice?->date)?->addDays($due_date_roof_rule?->due_date)?->format('d M Y') }}"
                                    disabled>
                                @error('amount')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="row g-3">
                        @foreach ($categories as $category)
                            <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                <div class="form-label">Total
                                    <b>{{ empty($category) ? 'All' : Str::title(Str::replace('-', ' ', $category)) }}</b>
                                </div>
                                <input type="text" class="form-control"
                                    value="Rp. {{ number_format($amounts[$category], 2, ',', '.') }}" placeholder=""
                                    disabled>
                            </div>
                        @endforeach
                        @foreach ($categories as $category)
                            <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                ...
                                <input type="text" class="form-control" value="{{ $payment_amounts[$category] }}"
                        ...
                        @foreach ($categories as $category)
                            <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                ...
                                <input type="text" class="form-control"
                                    value="{{ $remaining_amounts[$category] }}" placeholder="" disabled>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="col-md-12">
            <div class="mb-4 card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <button type="submit" class="btn btn-success waves-effect waves-light" data-bs-toggle="modal"
                        data-bs-target="#modal-v2">Tambah </button>
                </div>
                        ...
                        <tbody>
                            @forelse ($invoice_details as $result)
                                <tr>
                                    ...
                                    <td class = "text-center">
                                        {{ $result?->date ? $result?->date?->format('d M Y') : '-' }}</td>
                                    <td class = "text-center">{{ $result?->percentage }}</td>
                                    <td class="text-center">
                                        <button class="btn btn-danger btn-sm"
                                            wire:click="deleteConfirm('{{ $result?->id }}')" x-data="{ tooltip: 'Hapus' }"
                                            x-tooltip="tooltip"><i class="fa-solid fa-trash-alt fa-fw"></i></button>
                                        <button class="btn btn-warning btn-sm"
                                            wire:click="edit('{{ $result?->id }}')" x-data="{ tooltip: 'Edit' }"
                                            x-tooltip="tooltip"><i class="fa-solid fa-pencil-alt fa-fw"></i></button>
