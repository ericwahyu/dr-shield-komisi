                    ...
                    <div class="row g-3">
                        @foreach ($categories as $category)
                            <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                <div class="form-label">Total <b>{{ empty($category) ? 'All' : Str::title(Str::replace('-', ' ', $category)) }}</b></div>
                                <input type="text" class="form-control" value="Rp. {{ number_format($amounts[$category], 0, ',', '.') }}" placeholder="" disabled>
                            </div>
                        @endforeach
                        @foreach ($categories as $category)
                            <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                <div class="form-label">Nominal Terbayar</div>
                                <input type="text" class="form-control" value="{{ $payment_amounts[$category] }}" placeholder="" disabled>
                            </div>
                        @endforeach
                        @foreach ($categories as $category)
                            <div class="col-{{ 12 / (count($categories) > 0 ? count($categories) : 1) }}">
                                <div class="form-label">Sisa Pembayaran</div>
                                <input type="text" class="form-control" value="{{ $remaining_amounts[$category] }}" placeholder="" disabled>
                            </div>
                        @endforeach
                    </div>
                </div>
