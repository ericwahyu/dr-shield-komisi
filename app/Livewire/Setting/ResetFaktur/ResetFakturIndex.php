...
class ResetFakturIndex extends Component
{
        ...
        return view('livewire.setting.reset-faktur.reset-faktur-index', [
            ...
            // })->select('id', 'name')->orderBy('name', 'ASC')->get(),
             ...
             'list_secondary' => User::role('sales')->whereHas('userDetail', function ($query) {
                $query->when($this->sales_type, function ($query) {
                    $query->where('sales_type', $this->sales_type);
                });
    ...
    public function saveData()
    {
        $this->validate(
            ...
            [
                'date_reset.required' => 'Tanggal data import yang akan di reset wajib diisi',
                // 'sales_type' => 'Tipe wajib diisi',
            ]
        );
        ...
        try {
            DB::beginTransaction();
                ...
                $invoice_ids = Invoice::whereYear('date', Carbon::parse($this->data_reset)->year)->whereMonth('date', Carbon::parse($this->data_reset)->month)
                ->when($this->sales_type != null, function ($query) {
                    $query->where('type', $this->sales_type);
                })
                ...
                $commission_ids = Commission::where('year', Carbon::parse($this->data_reset)->year)->where('month', Carbon::parse($this->data_reset)->month)
                 ->when($this->sales_type != null, function ($query) {
                     $query->whereHas('user.userDetail', function ($query) {
                        $query->where('sales_type', $this->sales_type);
                    });
                })
                ->when($this->sales_id != null, function ($query) {
                ...
                InvoiceDetail::whereYear('date', Carbon::parse($this->data_reset)->year)->whereMonth('date', Carbon::parse($this->data_reset)->month)
                ->when($this->sales_type != null, function ($query) {
                ...
                Invoice::whereYear('date', Carbon::parse($this->data_reset)->year)->whereMonth('date', Carbon::parse($this->data_reset)->month)
                ->when($this->sales_type != null, function ($query) {
                    $query->where('type', $this->sales_type);
                })
                ->when($this->sales_id != null, function ($query) {
