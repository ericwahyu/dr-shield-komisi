...
class CeramicCommissionDetail extends Component
{
    ...
    public function mount($sales_id, $month_commission)
    {
        ...
        })->where('year', (int)Carbon::parse($month_commission)?->format('Y'))->where('month', (int)Carbon::parse($month_commission)?->format('m'))->whereNull('category_id')->where('version', 1)->first();
