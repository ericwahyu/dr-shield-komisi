...
namespace App\Livewire\Invoice\CeramicInvoice\CeramicInvoiceDetail;
...
use App\Traits\InvoiceDetailProcess;
...
class CeramicInvoiceDetailV2 extends Component
{
    use LivewireAlert, WithPagination, GetSystemSetting, CeramicInvoiceProsses, CeramicInvoiceDetailProsses, CeramicCommissionDetailProsses, CommissionProcess;
    ...
    {
        $this->payment_amount   = "Rp. ". number_format((int)$this->get_invoice->invoiceDetails()->where('version', 2)->sum('amount'), 0, ',', '.');
        $this->remaining_amount = "Rp. ". number_format((int)$this->get_invoice?->amount - (int)$this->get_invoice->invoiceDetails()->where('version', 2)->sum('amount'), 0, ',', '.');
        return view('livewire.invoice.ceramic-invoice.ceramic-invoice-detail.ceramic-invoice-detail-v2', [
    ...
    public function mount($id)
    {
        ...
        $this->due_date_ceramic_rules = $this->get_invoice->dueDateRules()->where('version', 2)->get();
        $this->type                   = 'ceramic';
    }
    ...
    public function saveData()
    {
        ...
        try {
            DB::transaction(function () {
                ...
                $datas = array(
                    'version'             => 2,
                    'income_tax'          => $this->get_invoice?->income_tax,
                    'invoice_detail_date' => $this->invoice_detail_date,
                );
